...
export default {
  ...
  methods: {
    ...
    async guardarDatos(data) {
      try {
        const res = await this.$administrador.$post(this.apiUrl, data);
        if (res.success) {
    ...
    async saveData() {
      ...
      // Esto podría implicar hacer una solicitud POST/PUT a tu administrador
      try {
        const response = await this.$administrador.$post(this.apiUrl, this.model);
