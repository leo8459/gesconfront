  ...
  methods: {
    ...
    async subirArchivo() {
      ...
      try {
        ...
        const response = await this.$api.$post('/solicitudes2/carga-masiva', formData);
  ...
  Swal.fire({
    ...
    async generateGuideNumber() {
      try {
        ...
        const response = await this.$api.$post('/generar-guia', {
...
,
  ...
  if (user && user.user) {
    // Eliminar la asignación automática
