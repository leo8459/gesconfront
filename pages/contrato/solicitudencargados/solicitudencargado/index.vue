  ...
  methods: {
    async markAsEnCamino(solicitudeId) {
      ...
      try {
        ...
        const response = await this.$contratos.$put(`solicitudesrecojo/${solicitudeId}`, { cartero_recogida_id: carteroId });
    ...
    async collectSelected() {
      ...
      try {
        ...
        for (let item of this.selectedItemsData) {
          await this.$contratos.$put(`marcarrecogido/${item.id}`, { cartero_recogida_id: carteroId });
        }
        await this.GET_DATA(this.apiUrl); // Forzar actualización de la lista
        this.$swal.fire({
          icon: 'success',
  ...
  mounted() {
    this.$nextTick(async () => {
      ...
      try {
        ...
        const sucursales = await this.$contratos.$get('sucursales');
