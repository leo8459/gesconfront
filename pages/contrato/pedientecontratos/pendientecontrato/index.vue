                ...
                <tbody>
                  <tr v-for="(m, i) in paginatedData" :key="m.id">
                    ...
                    <td class="py-0 px-1">{{ dash(m.fecha) }}</td>
                        ...
                        <td class="py-0 px-1">{{ dash(m.fecha_envio_regional) }}</td>
...
export default {
  ...
  computed: {
    ...
    const filtered = this.list.filter(item => {
      ...
      const isMatchingState = [1,3,5,6,7,8,9,10,11,12,13].includes(item.estado);
      ...
      if (startDate || endDate) {
        ...
        if (item.fecha) {
          datesToCheck.push(new Date(item.fecha));
        }
        if (item.fecha_recojo_c) {
          datesToCheck.push(new Date(item.fecha_recojo_c));
        }
        if (item.fecha_envio_regional) {
          datesToCheck.push(new Date(item.fecha_envio_regional));
        }

        if (datesToCheck.length === 0) {
          // Si no hay fechas, excluir el item
      ...
      const result = isMatchingSucursal && isMatchingOrigen && isMatchingDepartamento && isMatchingState && isWithinDateRange && matchesSearchTerm;
    ...
    filtered.sort((a, b) => {
      const getLatestDate = (item) => {
        const dates = [];
        if (item.fecha) dates.push(new Date(item.fecha));
        if (item.fecha_recojo_c) dates.push(new Date(item.fecha_recojo_c));
        if (item.fecha_envio_regional) dates.push(new Date(item.fecha_envio_regional));
  ...
  // ... (resto de tus propiedades computadas)
...
async loadImageAsBase64(path) {
  ...
  mounted() {
    ...
    this.list = this.list.filter(item =>item.estado === 1 ||item.estado === 3  ||item.estado === 5 ||item.estado === 6 ||item.estado === 7 ||item.estado === 8 ||item.estado === 9 ||item.estado === 10 ||item.estado === 11 || item.estado === 12 || item.estado === 13); // Cambia aquí los estados a 8 y 9
